...
//                mist($rtn,$ssql,$bind,$this->getErrorMessage());
...
//        var_dump($rtn,$ssql,$bind,$this->getErrorMessage());
    ...
    /**
     * 改造自父类方法getTableList
     * 当查询中使用了group by的时候，需要使用它来获取概览下的记录数目
     * @param $csql
     * @param $ssql
     * @param $bind
     * @return array|string
...
//********************************** TODO:EXCEL IO ****************************************************--//
    ...
    public function initPHPExcel(){
        ...
        isset($this->objPHPExcel) or $this->objPHPExcel = new PHPExcel();
    }
     ...
     */
    public function setExcelBigTitle($title,$colnum,$style=null){
        //合并单元格
        $this->objPHPExcel->getActiveSheet(0)->mergeCells('A1:' . $colnum . '1');
        //设置样式
        $this->objPHPExcel->getActiveSheet()->getStyle('A1')->applyFromArray(isset($style)?$style:$this->titleStyle);
    ...
    protected function chr($num){
        ...
        if(!isset($cache[$num])){
            ...
            if($gap > 0){//是否超过一个'Z'的限制
                ...
                $cache[$num] = $this->chr(ord('A') + $piecenum).chr(64+$gap - $piecenum * 26);
            }else{
                $cache[$num] = chr($num);
            }
        }
    ...
    public function fullyExportExcelFile($data, $opfilename = null) {
        ...
        /*-- 设置大标题 --*/
        $this->setExcelBigTitle($data['title'],self::$cellName[$cellNum - 1],isset($data['titledstyle'])?$data['titledstyle']:null);
        ...
        for ($i = 0; $i < $dataNum; $i++) {
            ...
            for ($j = 0; $j < $cellNum; $j++) {
                ...
                //值与风格设置
                $valueObj = $this->objPHPExcel->setActiveSheetIndex(0);
                switch($titletype[$j]) {
                    case self::TYPE_NUM:
